...
function App() {
  ...
  return <main>
    ...
    <section className="toolbar">
      ...
      <div className="mode"><button disabled={Boolean(actionBusy)} className={!penMode&&camera?.mode==="GENERIC_GESTURE"?"active":""} onClick={()=>changeMode("GENERIC_GESTURE")}>제스처</button><button disabled={Boolean(actionBusy)} className={penMode?"active":""} onClick={togglePenMode}>펜</button><button disabled={Boolean(actionBusy)} className={drowsyMode?"active safety":""} onClick={toggleDrowsinessMode}>졸음 방지</button><button disabled={Boolean(actionBusy)} className={!penMode&&camera?.mode==="DIAGNOSTIC"?"active":""} onClick={()=>changeMode("DIAGNOSTIC")}>진단</button><button disabled={Boolean(actionBusy)} className={!penMode&&camera?.mode==="IDLE"?"active":""} onClick={()=>changeMode("IDLE")}>대기</button></div>
