...
describe("운전 화면의 실제 눈 위치 표시", () => {
  it("눈 양끝의 실제 좌표를 카메라 미러 좌표로 변환한다", () => {
    const face = Array.from({ length: 468 }, () => ({ x: 0, y: 0, z: 0 }));
    face[33] = { x: .2, y: .4, z: 0 }; face[133] = { x: .3, y: .4, z: 0 };
    const rings = getDriverEyeRings(face, 1000, 600);
    expect(rings).toHaveLength(1);
    expect(rings[0].x).toBe(750);
    expect(rings[0].y).toBe(240);
    expect(rings[0].radiusX).toBeCloseTo(78);
  });
  it("기울어진 얼굴에서도 화면 픽셀 비율에 맞게 렌즈가 회전한다", () => {
    const face = Array.from({ length: 468 }, () => ({ x: 0, y: 0, z: 0 }));
    face[33] = { x: .2, y: .3, z: 0 }; face[133] = { x: .3, y: .4, z: 0 };
    expect(getDriverEyeRings(face, 1000, 500)[0].angle).toBeCloseTo(Math.atan2(50, -100));
  });
  it("측정점 또는 화면 크기가 없으면 가짜 렌즈를 만들지 않는다", () => {
    expect(getDriverEyeRings([], 640, 480)).toEqual([]);
    expect(getDriverEyeRings([], 0, 480)).toEqual([]);
  });
});
