...
2026-09-11: **0.1.19 / versionCode 20**은 거치 안내 그림에 핸들·계기판을 추가하고 핸들 앞쪽/오른쪽 상단 거치 위치를 표시합니다. [그림 수정 내용](docs/DRIVER_PLACEMENT_GUIDE.md).

2026-09-11: Wake Drive **0.1.18 / versionCode 19**에 Stitch 그림으로 휴대폰 거치 위치·전면 카메라 각도·얼굴 프레임을 설명하는 시작 팝업을 추가했습니다. ‘확인’은 이번 안내만 닫고, ‘그만 보기’는 다음부터 숨깁니다. [안내 팝업 정책과 원본](docs/DRIVER_PLACEMENT_GUIDE.md).

2026-09-11: Wake Drive **0.1.17 / versionCode 18**은 한국어 모드의 보관함·적용 영상·랜덤 경고에서 휴게소 안내요정을 제외합니다. 영어에서는 해당 영상만 제공하며 보유 기록과 기존 광고 해제 절차는 유지합니다. [언어별 영상 정책](docs/LANGUAGE_AND_ENGLISH_ALERTS.md).

2026-09-11: Wake Drive **0.1.16 / versionCode 17**은 라임색 안경·분홍색 미소 캐릭터로 런처 아이콘, 스플래시, 홈·측정 화면의 주요 아이콘을 통일했습니다. [브랜드 원본·Stitch 시안·재생성 방법과 검증 범위](docs/BRAND_IDENTITY.md)를 참고하세요.

2026-09-11: Wake Drive **0.1.15 / versionCode 16**은 영어 모드에 **휴게소 안내요정(video-6)** 한 편만 표시·재생하며 촬영 중 한국어·영어 변경을 허용합니다. 잠금 해제는 기존 광고 절차를 따릅니다. [언어·영어 경고 정책](docs/LANGUAGE_AND_ENGLISH_ALERTS.md), [0.1.14 카메라 일시정지 복구와 멈춤 감지 수정](docs/CAMERA_PLAYBACK.md)을 참고하세요. 아래 0.1.12 기록은 이전 검증 이력입니다.
...
- `scripts/branding.mjs`: 앱별 런처 아이콘, Wake Drive 스플래시, manifest, 서비스 워커
