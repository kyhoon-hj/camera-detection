...
export function StudyModeScreen({ onExit, alertVideoPath, endingVideoPath, adsRemoved = false }: { onExit(): void; alertVideoPath: string; endingVideoPath: string; adsRemoved?: boolean }) {
  const [settings, setSettings] = useState<StudySettings>(() => loadStudySettings(appStorage.getItem(STUDY_SETTINGS_STORAGE_KEY)));
  const [records, setRecords] = useState<StudySessionRecord[]>(() => loadStudyRecords(appStorage.getItem(STUDY_RECORDS_STORAGE_KEY)));
  const [rewardState, setRewardState] = useState<StudyRewardState>(() => loadStudyRewardState(appStorage.getItem(STUDY_REWARDS_STORAGE_KEY)));
  ...
  const saveSettings = useCallback((patch: Partial<StudySettings>) => {
    setSettings((current) => {
      ...
      appStorage.setItem(STUDY_SETTINGS_STORAGE_KEY, JSON.stringify(next));
  ...
  const updateRewardState = useCallback((updater: (current: StudyRewardState) => StudyRewardState) => {
    setRewardState((current) => {
      ...
      appStorage.setItem(STUDY_REWARDS_STORAGE_KEY, JSON.stringify(next));
      return next;
    });
  }, []);
  ...
  const resetStudyCalendar = useCallback(() => {
    ...
    appStorage.setItem(STUDY_RECORDS_STORAGE_KEY, JSON.stringify([]));
    setRecords([]);
  ...
  const finishSession = useCallback(() => {
    ...
    appStorage.setItem(STUDY_RECORDS_STORAGE_KEY, JSON.stringify(nextRecords));
  ...
  useEffect(() => {
    ...
    const listener = CapacitorApp.addListener("backButton", () => {
      if (trackerRef.current && !["START", "RESULT", "RECORD_DETAIL", "HISTORY", "SETTINGS"].includes(view)) { requestEnd(); return; }
      if (view !== "START") {
        stopCamera();
        setView("START");
  ...
  }, [onExit, requestEnd, stopCamera, view]);
