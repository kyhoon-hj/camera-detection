...
export function drawLandmarks(
  ...
  if (frame.face !== null) {
    ...
    if (comicDriverLens) {
      ...
      context.strokeStyle = alert ? "#ff8ab9" : "#d2f000";
      context.shadowColor = context.strokeStyle;
      context.shadowBlur = 7;
      for (const ring of getDriverEyeRings(frame.face, width, height)) {
        context.beginPath();
        context.ellipse(ring.x, ring.y, ring.radiusX, ring.radiusY, ring.angle, 0, Math.PI * 2);
        context.stroke();
      }
...
export function getDriverEyeRings(face: Landmark[], width: number, height: number) {
  if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) return [];
  return [[33, 133], [362, 263]].flatMap(([a, b]) => {
    const left = face[a], right = face[b];
    if (!left || !right || ![left.x, left.y, right.x, right.y].every(Number.isFinite)) return [];
    const x1 = normalizedXToMirroredCanvas(left.x, width), y1 = left.y * height;
    const x2 = normalizedXToMirroredCanvas(right.x, width), y2 = right.y * height;
    const distance = Math.hypot(x2 - x1, y2 - y1);
    if (distance < 1) return [];
    return [{ x: (x1 + x2) / 2, y: (y1 + y2) / 2, radiusX: distance * .78, radiusY: distance * .58, angle: Math.atan2(y2 - y1, x2 - x1) }];
  });
}
