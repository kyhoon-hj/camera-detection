...
- 위치 권한/GPS와 Firebase Analytics를 다시 도입하지 않는다. 2026-09-11 사용자 요청으로 로컬 ID를 사용하는 AdMob SDK와 광고용 INTERNET/ACCESS_NETWORK_STATE/AD_ID 권한을 복구한다. 광고는 홈과 감지 시작 전 전환에만 표시하며 기본 영상은 무료로 유지한다. 모델/WASM/영상은 앱 내 파일만 사용한다. 출시 서명키는 외부 환경변수로 설정한다.
