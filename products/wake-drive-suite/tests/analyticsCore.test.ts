...
describe('usage analytics privacy and aggregation', () => {
  ...
  it('counts monitoring time without location and excludes suspension gaps', () => {
    const emit=vi.fn(); const drive=new DriveMetrics(emit);
    drive.start(0); drive.start(0);
    drive.sample(1000); drive.sample(2000); drive.sample(3000); drive.sample(9000); drive.sample(10000);
    ...
    expect(emit.mock.calls.at(-1)?.[1]).toEqual({measured_seconds:4});
  });
  it('reports each minute and remaining tail without double counting progress', () => {
    ...
    for(let i=1;i<=65;i++) drive.sample(i*1000);
    drive.stop();
    ...
    expect(emit.mock.calls.at(-1)?.[1]).toEqual({measured_seconds:65});
  });
  it('consent revocation discards unfinished aggregates', () => {
    const emit=vi.fn();const drive=new DriveMetrics(emit);drive.start(0);drive.sample(1000);drive.discard();drive.stop();
