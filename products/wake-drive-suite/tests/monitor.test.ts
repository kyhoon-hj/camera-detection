...
import { getForcedWakeUpReason, getWakeUpCountActivity } from "../src/wakeUpVideos";

describe("운전 고개 숙임 오탐 회귀", () => {
  function setup(pose: Landmark[] | null, mode: "DROWSINESS" | "STUDY" = "DROWSINESS", initialNoseDrop = 0) {
    const monitor = new DriverMonitor();
    monitor.begin(mode);
    for (let time = 0; time <= 5_000; time += 100) monitor.process(sample(time, initialNoseDrop, pose));
    return monitor;
  }
  function sample(timestampMs: number, noseDrop: number, pose: Landmark[] | null, closed = false): VisionFrame {
    const face = makeFace(closed);
    face[1].y += noseDrop;
    return { timestampMs, face, pose };
  }

  it.each(["pose", "face-only"])("%s: 작은 숙임을 5초 유지해도 영상 카운트나 강제 경고를 만들지 않는다", (source) => {
    const monitor = setup(source === "pose" ? makePose() : null);
    const pose = source === "pose" ? makePose({ verticalDrop: 0.04 }) : null;
    for (let time = 5_100; time <= 10_100; time += 100) {
      const snapshot = monitor.process(sample(time, source === "pose" ? 0.05 : 0.06, pose));
      expect(snapshot.headDown).toBe(false);
      expect(getWakeUpCountActivity(snapshot).head).toBe(false);
      expect(getForcedWakeUpReason(snapshot)).toBeNull();
    }
  });

  it.each(["pose", "face-only"])("%s: 충분한 숙임은 2초 카운트와 4초 경고를 유지하고 작은 숙임으로 돌아오면 해제한다", (source) => {
    const monitor = setup(source === "pose" ? makePose() : null);
    const pose = source === "pose" ? makePose({ verticalDrop: 0.10 }) : null;
    for (let time = 5_100; time <= 9_100; time += 100) {
      const snapshot = monitor.process(sample(time, 0.10, pose));
      if (time === 7_100) expect(getWakeUpCountActivity(snapshot).head).toBe(true);
      if (time === 9_100) expect(getForcedWakeUpReason(snapshot)).toBe("HEAD");
    }
    const recovered = monitor.process(sample(9_200, 0.04, source === "pose" ? makePose({ verticalDrop: 0.03 }) : null));
    expect(recovered.headDownDurationMs).toBe(0);
    expect(recovered.headDown).toBe(false);
    expect(getForcedWakeUpReason(recovered)).toBeNull();
  });

  it("600ms의 짧은 끄덕임은 확정하지 않고 다음 숙임에 시간을 합치지 않는다", () => {
    const monitor = setup(null);
    for (let time = 5_100; time <= 5_700; time += 100) {
      expect(monitor.process(sample(time, 0.10, null)).headDown).toBe(false);
    }
    monitor.process(sample(5_800, 0, null));
    for (let time = 5_900; time <= 6_600; time += 100) {
      expect(monitor.process(sample(time, 0.10, null)).headDown).toBe(false);
    }
    const confirmed = monitor.process(sample(6_700, 0.10, null));
    expect(confirmed.headDown).toBe(true);
    expect(confirmed.headDownDurationMs).toBe(800);
  });

  it("기본 위치를 다시 저장하면 해당 카메라 각도에서의 작은 추가 움직임은 정상이다", () => {
    const monitor = setup(null);
    monitor.recalibrate();
    for (let time = 5_100; time <= 10_100; time += 100) monitor.process(sample(time, 0.08, null));
    for (let time = 10_200; time <= 15_200; time += 100) {
      expect(monitor.process(sample(time, 0.14, null)).headDown).toBe(false);
    }
  });

  it("작은 숙임에서도 눈 감김은 기존 시간으로 독립 경고한다", () => {
    const monitor = setup(null);
    let snapshot = monitor.process(sample(5_100, 0.06, null, true));
    for (let time = 5_200; time <= 9_100; time += 100) snapshot = monitor.process(sample(time, 0.06, null, true));
    expect(snapshot.headDown).toBe(false);
    expect(snapshot.closedDurationMs).toBe(4_000);
    expect(getForcedWakeUpReason(snapshot)).toBe("EYES");
  });

  it("열공의 기존 고개 숙임 판정은 운전 조정의 영향을 받지 않는다", () => {
    const monitor = setup(makePose(), "STUDY");
    let snapshot = monitor.process(sample(5_100, 0.05, makePose({ verticalDrop: 0.04 })));
    for (let time = 5_200; time <= 7_100; time += 100) snapshot = monitor.process(sample(time, 0.05, makePose({ verticalDrop: 0.04 })));
    expect(snapshot.headDown).toBe(true);
    expect(snapshot.headDownDurationMs).toBe(2_000);
  });
});
