...
if (!['build','android'].includes(action) || selection!=='jolbang') throw new Error('Usage: node scripts/app.mjs build|android jolbang|yeolgong|all');
...
for(const variant of variants) {
 ...
 const result=spawnSync(process.platform==='win32'?'gradlew.bat':'./gradlew',['assembleDebug','--console=plain','--max-workers=2','--no-daemon'],{cwd:nativeRoot,env,stdio:'inherit',shell:process.platform==='win32'});
