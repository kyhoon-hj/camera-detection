const CACHE="jolbang:shell:1789117626775";
...
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('jolbang:shell:')&&key!==CACHE).map(key=>caches.delete(key)))));self.clients.claim();});
