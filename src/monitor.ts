...
// Normalized landmark ratios relative to calibration, not physical pitch angles.
// Driver thresholds reject small glances, including after a deeper head drop.
const DRIVER_HEAD_DOWN = {
  confirmMs: 800,
  faceEnter: 0.15,
  faceHold: 0.12,
  faceOnlyEnter: 0.18,
  faceOnlyHold: 0.15,
  heightDropEnter: 0.18,
  heightDropHold: 0.14,
} as const;
...
export class DriverMonitor {
  ...
  private monitor(frame: VisionFrame): MonitorSnapshot {
    ...
    const headDownCandidate = detectHeadDown(pitch, currentPose, baseline, this.headWasDown, sideFactor, this.mode);
    ...
    const headDown = headDownCandidate
      && this.headDownCandidateSince !== null
      && frame.timestampMs - this.headDownCandidateSince >= (this.mode === "DROWSINESS" ? DRIVER_HEAD_DOWN.confirmMs : HEAD_DOWN_CONFIRM_MS);
...
function detectHeadDown(
  ...
  const driver = mode === "DROWSINESS";
  const faceDelta = driver
    ? (wasDown ? DRIVER_HEAD_DOWN.faceHold : DRIVER_HEAD_DOWN.faceEnter)
    : (wasDown ? 0.065 : 0.09);
  const faceThreshold = baseline.pitch + faceDelta + combinedSideFactor * 0.06;
  ...
  if (!poseReliable) {
    const faceOnlyDelta = driver
      ? (wasDown ? DRIVER_HEAD_DOWN.faceOnlyHold : DRIVER_HEAD_DOWN.faceOnlyEnter)
      : (wasDown ? 0.085 : 0.12);
    const faceOnlyThreshold = baseline.pitch + faceOnlyDelta + combinedSideFactor * 0.10;
  ...
  const requiredDrop = driver
    ? (wasDown ? DRIVER_HEAD_DOWN.heightDropHold : DRIVER_HEAD_DOWN.heightDropEnter)
    : (wasDown ? 0.08 : 0.12);
