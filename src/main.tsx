...
import { DriverHud, useDriverSpeed } from "./DriverHud";
import { unlockWakeUpVideo } from "./unlockWakeUpVideo";
import { chooseWakeUpVideo, loadWakeUpPlaybackMode, WAKE_UP_PLAYBACK_STORAGE_KEY, type WakeUpPlaybackMode } from "./wakeUpPlayback";
...
function App() {
  ...
  initialWakeUpLibraryRef.current ??= loadWakeUpLibraryState(appStorage.getItem(WAKE_UP_LIBRARY_STORAGE_KEY));
  ...
  const driverSpeed = useDriverSpeed(APP.variant === "jolbang" && activeModule === "DROWSINESS" && gpsEnabled, compactPip);
  const analyticsStatus = useSyncExternalStore(subscribeAnalytics, getAnalyticsStatus);
  const analyticsSpeed = useRef<number | null>(null);
  analyticsSpeed.current = driverSpeed.kmh;
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => driveMetrics.sample(performance.now(), analyticsSpeed.current), 1000);
  ...
  useEffect(() => { if (runState === "READY" || runState === "ERROR" || activeModule !== "DROWSINESS") setGpsEnabled(false); }, [runState, activeModule]);
  useEffect(() => {
  ...
  const changeWakeUpPlaybackMode = useCallback((mode: WakeUpPlaybackMode) => {
    try {
      ...
      setWakeUpPlaybackFeedback("설정을 저장하지 못했습니다. 다시 시도해 주세요.");
      setWakeUpLibraryFeedback("설정을 저장하지 못했습니다. 다시 시도해 주세요.");
      return false;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const receive = (state: DriverPipState) => {
      ...
      if ((!state.visible || state.stopReason) && (streamRef.current !== null || startInFlight.current)) {
        stop();
        setError(state.stopReason === "stalled" ? "화면이나 측정 갱신이 멈춰 카메라를 종료했습니다. 다시 시작해 주세요." : state.stopReason === "user" ? "작은 화면에서 측정을 종료했습니다." : "작은 화면이 닫히거나 앱이 숨겨져 측정을 종료했습니다.");
      }
    };
    const listener = driverPip.addListener("stateChanged", receive);
    void driverPip.getState().then(receive).catch(() => setPipFeedback("작은 화면 기능을 불러오지 못했습니다. 최신 APK를 설치해 주세요."));
    return () => { disposed = true; void listener.then(handle => handle.remove()); };
  }, [nativePip, stop]);

  useEffect(() => {
    if (!nativePip) return;
    void driverPip.setSession({ running: runState === "RUNNING" && activeModule === "DROWSINESS", enabled: pipEnabled }).catch(() => setPipFeedback("작은 화면 연결을 확인해 주세요. 앱 화면에서 측정을 계속할 수 있습니다."));
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const progressing = wakeUpVideoPlayingRef.current
  ...
  const enterPip = async () => {
    ...
    catch (cause) { setPipFeedback(cause instanceof Error ? cause.message : "작은 화면을 열지 못했습니다. Android의 화면 속 화면 허용 설정을 확인해 주세요."); }
  };

  const enableGps = async () => {
    try { if (nativeDriverPermissions && !(await driverPermissions.ensure({feature:"location"})).allowed) return; setGpsEnabled(true); }
    catch { setError("위치 권한을 확인하지 못했습니다. 권한 확인 및 설정에서 확인해 주세요."); }
  };

  useEffect(() => {
    if (!nativePip) return;
    void driverPermissions.configure({location:gpsEnabled,pip:pipEnabled}).catch(()=>{});
  }, [nativePip,gpsEnabled,pipEnabled]);
    ...
    if (video && canvas && engine && !recoveryRef.current && isUsableVideoFrame(video) && hasNewFrame && now - lastInferenceRef.current >= 80) {
      ...
      try {
        ...
        if (wakeUpReason !== null && !wakeUpVideoPlayingRef.current) {
          const selectedVideo = chooseWakeUpVideo(wakeUpLibraryRef.current, APP.variant === "jolbang" ? wakeUpPlaybackModeRef.current : "APPLIED");
          ...
          analyticsEvent("drive_warning", {video_id:selectedVideo.id, reason:wakeUpReason});
  ...
  const start = useCallback(async (): Promise<boolean> => {
    ...
    setError("");
    setStartupMessage("카메라 권한 확인 중…");
    try {
      if (nativeDriverPermissions) {
        const access = await withStartupTimeout(driverPermissions.ensure({feature:"camera"}),8000,"권한 상태를 확인하지 못했습니다. 앱을 다시 열어 주세요.");
        if (generation !== startGeneration.current) return false;
        if (!access.allowed) { setCameraPermission("DENIED"); throw new Error("카메라 권한이 꺼져 있습니다. ‘권한 확인 및 설정’에서 카메라를 허용해 주세요."); }
      }
      if (soundEnabled) {
      ...
      setStartupMessage("카메라 연결 중… 권한 창이 나오면 허용해 주세요.");
      ...
      setStartupMessage("카메라 영상 준비 중…");
      await withStartupTimeout(videoRef.current.play(),8000,"카메라 영상 재생이 지연됐습니다. 다른 카메라 앱을 닫고 다시 시작해 주세요.");
      await waitForUsableVideoFrame(videoRef.current);
      if (generation !== startGeneration.current) return false;
      const engine = new MobileVisionEngine();
      analyticsStage="analysis_setup";
      setStartupMessage("졸음 분석 준비 중… 잠시만 기다려 주세요.");
      ...
      await withStartupTimeout(initialization,35000,"졸음 분석 준비가 지연됐습니다. 감지를 종료하고 다시 시작해 주세요.");
  ...
  const reportWakeUpPlaybackFailure = useCallback(() => {
    ...
    finishWakeUpVideo();
    setWakeUpPlaybackError("경고 영상을 재생하지 못했습니다. 카메라 감지는 계속됩니다. 안전한 곳에 정차한 뒤 보관함의 영상 미리보기를 확인해 주세요.");
    beep(audioRef.current, true);
    void speakKorean("졸음 경고입니다. 안전한 곳에 정차하고 쉬어 주세요.").catch(() => undefined);
  }, [finishWakeUpVideo]);
  ...
  const playWakeUpVideo = useCallback(async () => {
    ...
    try {
      ...
      alertVideo.volume = 1;
      await withStartupTimeout(alertVideo.play(), 3000, "경고 영상 재생 지연");
    } catch {
      if (generation === startGeneration.current) reportWakeUpPlaybackFailure();
    }
  ...
  const downloadSelectedWakeUpVideo = useCallback(async (requestedId?: WakeUpVideoId) => {
    ...
    if (downloaded) {
      ...
      if (APP.variant === "jolbang") setWakeUpLibraryFeedback(`${profile.name} · 현재 적용 영상만 재생합니다.`);
    ...
    if (APP.variant === "jolbang" && !Capacitor.isNativePlatform()) {
      setWakeUpLibraryFeedback("잠금 해제는 Android 앱에서 광고 시청 후 이용할 수 있어요. 웹에서는 광고 보상을 확인할 수 없어 잠금이 유지됩니다.");
      return;
    }
    wakeUpDownloadInFlight.current = true;
    setWakeUpDownloadBusy(true);
    analyticsEvent("reward_unlock_start", {video_id:profile.id});
    setWakeUpLibraryFeedback("보상형 동영상 광고를 끝까지 시청하면 영상 다운로드가 시작됩니다.");
    try {
      const next = await unlockWakeUpVideo(wakeUpLibrary, profile.id, {
        reward: async () => {
          try {
            const rewarded = await showRewardedDownloadAd(profile.id);
            analyticsEvent("reward_ad_result", {video_id:profile.id, outcome:rewarded ? "rewarded" : "cancelled"});
            return rewarded;
          } catch (cause) {
            analyticsEvent("reward_ad_result", {video_id:profile.id, outcome:"error"});
            throw cause;
          }
        },
        download: async () => {
          const response = await fetch(profile.path, { cache: "force-cache" });
          if (!response.ok) throw new Error("영상 파일을 받지 못했습니다. 잠금이 유지됩니다.");
          if ("caches" in window) {
            const cache = await window.caches.open(cacheName("suha-wake-up-videos-v1"));
      ...
      if (!next) {
        setWakeUpLibraryFeedback("광고 시청 보상이 확인되지 않아 잠금이 유지됩니다. 광고를 끝까지 본 뒤 다시 시도해 주세요.");
        return;
      }
      persistWakeUpLibrary(next);
      setPendingWakeUpApplyId(profile.id);
      analyticsEvent("video_unlocked", {video_id:profile.id});
      setWakeUpLibraryFeedback(`${profile.name} 잠금 해제 완료 · ‘이 영상으로 설정하기’를 누르면 적용됩니다.`);
    } catch (cause) {
      setWakeUpLibraryFeedback(cause instanceof Error ? cause.message : "영상 다운로드 중 문제가 발생했습니다.");
      analyticsEvent("video_unlock_error", {video_id:profile.id});
    } finally {
      wakeUpDownloadInFlight.current = false;
    ...
    <main ref={appElementRef} onClickCapture={event => {
      ...
      <header className="topbar">
        <div className="brand-lockup">
          {activeTab === "SETTINGS" && <button data-analytics-action="close_settings" className="header-icon back" onClick={() => setActiveTab("MONITOR")} aria-label={activeModule === "HOME" ? "설정 닫고 홈으로" : "설정 닫고 측정 화면으로"}>
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 5 8 12l7 7" /></svg>
          </button>}
          {activeTab !== "SETTINGS" && activeModule !== "HOME" && activeModule !== "STUDY" && <button data-analytics-action="go_home" className="header-icon back" onClick={goHome} aria-label="이전 화면">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 5 8 12l7 7" /></svg>
          </button>}
          {activeModule !== "STUDY" && <div className="brand" aria-label={APP.name}>
            <img src="/icon-192.png" alt="" />
            {APP.variant === "jolbang" ? <span className="wake-brand-copy"><span className="wake-brand-title">{APP.name}<i>COMIC!</i></span><small>{activeModule === "HOME" ? "⚡ 출발 전, 잠깨우기 준비" : runState === "RUNNING" ? "눈 감김 · 고개 움직임 감지 중" : "카메라로 살피는 졸음 신호"}</small></span> : <span>{APP.name}</span>}
          </div>}
        </div>
        {APP.variant === "jolbang" && activeModule !== "STUDY" && activeTab !== "SETTINGS" && <button data-analytics-action="open_settings" className="header-icon" onClick={() => setActiveTab("SETTINGS")} aria-label="설정 열기">⚙</button>}
      </header>

      {activeModule === "HOME" && activeTab !== "SETTINGS" && APP.variant === "jolbang" && <DriverLibrary
        library={wakeUpLibrary} selectedId={selectedWakeUpVideoId} busy={wakeUpDownloadBusy} playbackMode={wakeUpPlaybackMode}
        feedback={wakeUpLibraryFeedback} voiceFeedback={voiceFeedback} soundEnabled={soundEnabled}
        onSelect={id => { if (wakeUpDownloadInFlight.current) return; setSelectedWakeUpVideoId(id); setWakeUpLibraryFeedback(wakeUpLibrary.downloadedIds.includes(id) ? `${getWakeUpVideoProfile(id).name} 선택됨 · 아래 설정 버튼으로 적용해 주세요.` : `${getWakeUpVideoProfile(id).name} 잠김 · 광고를 끝까지 시청하면 해제됩니다.`); }}
      ...
      {activeModule === "DROWSINESS" && activeTab === "MONITOR" && <header className="driver-monitor-intro">
        <div><span>WAKE UP · DRIVE</span><h2>졸음 감지</h2></div>
        <b className={runState === "RUNNING" ? "is-live" : ""}><i />{runState === "RUNNING" ? calibrating ? "기준 측정" : "감지 중" : runState === "LOADING" ? "연결 중" : "시작 전"}</b>
      </header>}

      {activeModule === "DROWSINESS" && activeTab === "MONITOR" && !compactPip && <aside className="driver-pip-launch">
        <span>{nativePip ? pipState.supported ? pipEnabled ? "홈으로 나가도 작은 화면에서 측정해요." : "작은 화면 자동 전환이 꺼져 있어요." : "이 기기는 작은 화면을 지원하지 않아요." : "홈 화면 위 작은 창은 Android 앱에서 사용할 수 있어요."}</span>
        {nativePip && pipState.supported && pipEnabled && <button onClick={() => void enterPip()} disabled={runState !== "RUNNING"}>작은 화면으로 보기 ↗</button>}
      ...
      <section className={`camera-stage ${activeTab!=="MONITOR"?"mobile-screen-hidden":""}`} data-run-state={runState.toLowerCase()} aria-label={activeModule === "POSTURE" ? "자세 교정 카메라" : "운전자 카메라"}>
        <video ref={videoRef} playsInline muted />
        <canvas ref={canvasRef} />
        {compactPip && <div className="driver-pip-status"><b>{APP.name} · {wakeUpVideoPlaying ? "졸음 경고" : statusLabel}</b><span>{Math.floor((timelineSamples.at(-1)?.elapsedSeconds ?? 0) / 60)}분 측정</span></div>}
        {activeModule === "DROWSINESS" && !wakeUpVideoPlaying && <span className="driver-camera-label">기기 내 분석 · 영상 비저장</span>}
        ...
        {activeModule === "DROWSINESS" && !wakeUpVideoPlaying && !calibrating && <DriverHud
          speed={driverSpeed} seconds={runState === "RUNNING" ? timelineSamples.at(-1)?.elapsedSeconds ?? 0 : 0}
          running={runState === "RUNNING"} faceVisible={snapshot.faceVisible} enabled={gpsEnabled}
        />}
        <div className={`wake-up-overlay ${wakeUpVideoPlaying ? "playing" : ""}`} aria-hidden={!wakeUpVideoPlaying}>
          {activeModule === "DROWSINESS" && wakeUpVideoPlaying && <div className="driver-wake-caption"><b>WAKE UP!</b><span>졸음 경고 · 안전한 곳에서 쉬어 가세요</span></div>}
          <video
            ref={wakeUpVideoRef}
        ...
        {runState !== "RUNNING" && (
          <div className="camera-placeholder">
            {activeModule === "DROWSINESS" ? <div className="driver-lens-guide" aria-hidden="true">
              ...
              <svg viewBox="0 0 180 116" fill="none"><path d="M48 96C27 80 28 46 41 28C54 10 77 7 90 7C113 7 131 16 140 35C151 58 144 83 133 96" stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 6" /><rect x="29" y="40" width="51" height="39" rx="18" /><rect x="100" y="40" width="51" height="39" rx="18" /><path d="M80 55Q90 50 100 55" /><path className="driver-guide-eyes" d="M54 54V65M126 54V65" /><path className="driver-guide-smile" d="M78 90Q90 101 102 90" /></svg>
              <span className="driver-guide-note">눈과 고개가 잘 보이게</span>
      ...
      <section className={`status-panel ${activeTab==="SETTINGS"?"mobile-screen-hidden":""}`} aria-live="polite">
        {activeModule === "DROWSINESS" && activeTab === "MONITOR" && !compactPip && runState !== "RUNNING" && !gpsEnabled && <button className="driver-gps-enable" onClick={() => void enableGps()}>GPS 속도 표시 켜기 ↗</button>}
        <div className="status-heading">
          <div>
            <span className="eyebrow">{activeModule === "POSTURE" ? "POSTURE COACH" : "지금 나의 상태"}</span>
        ...
        {activeModule === "DROWSINESS" && <div className={`driver-alert-profile ${wakeUpVideoPlaying ? "is-playing" : ""}`}>
          <div className="driver-alert-art">{!wakeUpVideoPlaying && wakeUpPlaybackMode === "RANDOM_OWNED" ? <span className="driver-random-art" aria-hidden="true">⇄</span> : <VideoPoster id={wakeUpVideoPlaying ? playingWakeUpVideoId : wakeUpLibrary.appliedId} eager />}<span aria-hidden="true">!</span></div>
          <div><small>{wakeUpVideoPlaying ? "지금 재생 중인 경고 영상" : "경고 영상 재생 방식"}</small><strong>{wakeUpVideoPlaying ? getWakeUpVideoProfile(playingWakeUpVideoId).name : wakeUpPlaybackMode === "RANDOM_OWNED" ? `보유 영상 ${wakeUpLibrary.downloadedIds.length}편 랜덤 재생` : getWakeUpVideoProfile(wakeUpLibrary.appliedId).name}</strong></div>
          <b>{wakeUpVideoPlaying ? "재생 중" : wakeUpPlaybackMode === "RANDOM_OWNED" ? "랜덤" : "현재 영상만"}</b>
        </div>}
        ...
        <nav className={`controls ${activeTab!=="MONITOR"?"mobile-screen-hidden":""}`} aria-label="감지 제어">
          {runState === "RUNNING" ? (
            <>
              <button data-analytics-action="stop_detection" className="primary stop portrait-stop" onClick={stop}><span className="stop-icon" />감지 종료</button>
        ...
        {error && <div className="driver-start-feedback is-error" role="alert"><p>{error}</p>{nativePip && <button onClick={()=>void driverPermissions.showOverview()}>권한 확인 및 설정</button>}</div>}
        {wakeUpPlaybackError && <div className="driver-start-feedback is-error" role="alert"><p>{wakeUpPlaybackError}</p><button onClick={() => setWakeUpPlaybackError("")}>확인</button></div>}
        <DetectionTimeline
      ...
      {activeTab==="SETTINGS"&&<section className="mobile-settings" aria-label="앱 설정">
        ...
        {APP.variant === "jolbang" && <AnalyticsSettings />}
        {APP.variant === "jolbang" && <button data-analytics-action="toggle_pip" className="setting-row" aria-pressed={pipEnabled && nativePip && pipState.supported} disabled={!nativePip || !pipState.supported || runState === "RUNNING" || runState === "LOADING"} onClick={() => {
          ...
          catch { setPipFeedback("작은 화면 설정을 저장하지 못했습니다."); }
        }}><span><b>작은 화면으로 측정 유지</b><small>{nativePip ? pipState.supported ? "측정 중 홈으로 나가면 자동 전환돼요. 창을 닫거나 화면을 끄면 측정이 종료돼요." : "Android 8 이상이며 화면 속 화면을 지원하는 기기가 필요해요." : "Android 앱에서 사용할 수 있어요. 웹에서는 다른 앱으로 이동하면 측정이 종료돼요."}</small></span><i className={pipEnabled && nativePip && pipState.supported ? "on" : ""}>{nativePip && pipState.supported ? pipEnabled ? "켜짐" : "꺼짐" : "앱 전용"}</i></button>}
        {pipFeedback && <p className="voice-feedback" role="status">{pipFeedback}</p>}
        {APP.variant === "jolbang" && <fieldset className="driver-playback-settings" disabled={runState === "RUNNING" || runState === "LOADING" || wakeUpDownloadBusy}>
          <legend>경고 영상 재생 방식</legend>
          <label className="driver-playback-option"><input type="radio" name="wake-up-playback" value="APPLIED" checked={wakeUpPlaybackMode === "APPLIED"} onChange={() => changeWakeUpPlaybackMode("APPLIED")} /><span><b>현재 적용 영상만</b><small>{getWakeUpVideoProfile(wakeUpLibrary.appliedId).name} · 같은 영상으로 알려요.</small></span></label>
          <label className="driver-playback-option"><input type="radio" name="wake-up-playback" value="RANDOM_OWNED" checked={wakeUpPlaybackMode === "RANDOM_OWNED"} onChange={() => changeWakeUpPlaybackMode("RANDOM_OWNED")} /><span><b>보유 영상 랜덤</b><small>경고마다 잠금 해제한 {wakeUpLibrary.downloadedIds.length}편 중 무작위로 재생해요.</small></span></label>
          <p>{runState === "RUNNING" || runState === "LOADING" ? "재생 방식은 감지를 종료한 뒤 변경할 수 있어요." : wakeUpLibrary.downloadedIds.length === 1 ? "현재 보유 영상이 1편이라 랜덤에서도 같은 영상이 나와요." : "선택한 방식은 앱을 다시 열어도 유지돼요."}</p>
          {wakeUpPlaybackFeedback && <p role="alert">{wakeUpPlaybackFeedback}</p>}
        </fieldset>}
        {nativePip && <button data-analytics-action="open_permissions" className="setting-row" onClick={()=>void driverPermissions.showOverview()}><span><b>권한 확인 및 설정</b><small>카메라·위치·작은 화면 상태를 확인하고 휴대폰 설정으로 이동합니다.</small></span><i>열기 ↗</i></button>}
        {APP.variant === "jolbang" && <button data-analytics-action="toggle_gps" className="setting-row" onClick={() => gpsEnabled ? setGpsEnabled(false) : void enableGps()} aria-pressed={gpsEnabled}><span><b>GPS 참고 속도 표시</b><small>위치 권한을 사용합니다. 좌표는 저장하지 않으며 감지 종료 시 꺼집니다.</small></span><i className={gpsEnabled ? "on" : ""}>{gpsEnabled ? "켜짐" : "꺼짐"}</i></button>}
        <button data-analytics-action="toggle_sound" className="setting-row" onClick={()=>setSoundEnabled(value=>!value)} aria-pressed={soundEnabled}><span><b>경보음과 음성 안내</b><small>주의·위험 상태를 한국어로 알립니다.</small></span><i className={soundEnabled?"on":""}>{soundEnabled?"켜짐":"꺼짐"}</i></button>
        <button data-analytics-action="test_voice" className="setting-row" onClick={()=>void testVoice()}><span><b>안내 음성 테스트</b><small>기본 한국어 음성과 TTS 상태를 확인합니다.</small></span><i>재생</i></button>
        <button data-analytics-action="recalibrate" className="setting-row" onClick={()=>monitorRef.current.recalibrate()} disabled={runState!=="RUNNING"}><span><b>운전자 기준 재측정</b><small>{activeModule === "POSTURE" ? "얼굴과 자세 기준을 5초간 다시 측정합니다." : "눈과 고개 움직임 기준을 5초간 다시 측정합니다."}</small></span><i>재측정</i></button>
        ...
        <div className="privacy-card"><b>기기 내 개인정보 처리</b><p>카메라 영상은 서버로 보내거나 저장하지 않습니다. Android에서는 작은 화면이 보이는 동안 측정을 유지하며, 작은 창을 닫거나 화면을 끄면 종료합니다.</p></div>
      </section>}
      ...
      {activeTab==="SETTINGS"&&<button data-analytics-action="close_settings" className="back-to-monitor" onClick={() => setActiveTab("MONITOR")}>{activeModule === "HOME" ? "홈으로 돌아가기" : "측정 화면으로 돌아가기"}</button>}
...
function DetectionTimeline({ samples, mode }: { samples: TimelineSample[]; mode: TimelineMode }) {
  ...
  return <section className="timeline-panel" aria-label="시간 흐름 분석">
    <header className="timeline-heading">
      <div><span>SESSION TIMELINE</span><strong>{APP.variant === "jolbang" ? "졸음 신호 흐름" : "시간 흐름 분석"}</strong></div>
      <div className="timeline-status"><i className={samples.length ? riskClass : "pending"} />{samples.length ? `현재 ${riskLabel}` : "기록 대기"}</div>
    </header>
    ...
    {APP.variant === "jolbang" && <div className="driver-chart-time"><span>{samples.length ? "최근 기록" : "측정 전"}</span><span>현재</span></div>}
  ...
  return <div className="first-run-notice-backdrop">
    <section className="first-run-notice" role="dialog" aria-modal="true" aria-labelledby="first-run-notice-title" aria-describedby="first-run-notice-description">
      ...
      <div id="first-run-notice-description" className="first-run-notice-copy">
        ...
        <p>영상은 {APP.variant === "jolbang" ? "운전자 상태" : "학습 중 집중 상태"}를 확인하기 위해 화면에만 표시되며, 별도의 파일로 저장되지 않습니다.</p>
      </div>
...
function HomeScreen({
  ...
  return <section className={`mobile-home standalone-home ${APP.variant}`} aria-label={`${APP.name} 홈`}>
    <section className="wake-library" aria-label={`${APP.name} 영상 보관함`}>
      <header className="standalone-heading">
        <div><small>{APP.variant === "jolbang" ? "DRIVE SAFE" : "STUDY FOCUS"}</small><h1>{APP.variant === "jolbang" ? "출발 전, Wake Drive" : "오늘도, 열공"}</h1><p>{APP.description}</p></div>
      </header>
      <button
        type="button"
